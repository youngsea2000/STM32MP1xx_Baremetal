/**
  ...
  *          mounted on STM32MP135F-DK board.
  ...
  --------------------------
     ...
     - This driver uses timing and setting for EK79001 LCD.
...
/** @addtogroup STM32MP1XX_ATK
  * @{
  */

/** @addtogroup STM32MP1XX_ATK_LCD
  * @{
  */

/** @defgroup STM32MP1XX_ATK_LCD_Private_TypesDefinitions STM32MP1XX_ATK_LCD Private Types Definitions
  * @{
  */
...
/**
  * @}
  */
/** @defgroup STM32MP1XX_ATK_LCD_Private_Defines STM32MP1XX_ATK LCD Private Defines
  * @{
  */
...
/** @defgroup STM32MP1XX_ATK_LCD_Private_Macros STM32MP1XX_ATK_LCD Private Macros
  * @{
  */
...
/** @defgroup STM32MP1XX_ATK_LCD_Private_Variables STM32MP1XX_ATK_LCD Private Variables
  * @{
  */
...
/** @defgroup STM32MP1XX_ATK_LCD_Private_FunctionPrototypes STM32MP1XX_ATK_LCD Private Function Prototypes
  * @{
  */
...
/** @defgroup STM32MP1XX_ATK_LCD_Exported_Functions STM32MP1XX_ATK_LCD Exported Functions
  * @{
  */

/**
  * @brief  Get the LCD ID.
  * @param  Instance    LCD Instance
  * @retval LCD state
  */
uint8_t BSP_LCD_Get_ID(uint32_t Instance)
{
  uint8_t id = 0;
  ...
  LCD_ID_M0_GPIO_CLK_ENABLE();
  LCD_ID_M1_GPIO_CLK_ENABLE();
  LCD_ID_M2_GPIO_CLK_ENABLE();
  ...
  gpio_init_structure.Pin = LCD_ID_M0_PIN;
  HAL_GPIO_Init(LCD_ID_M0_GPIO_PORT, &gpio_init_structure);
  gpio_init_structure.Pin = LCD_ID_M1_PIN;
  HAL_GPIO_Init(LCD_ID_M1_GPIO_PORT, &gpio_init_structure);
  gpio_init_structure.Pin = LCD_ID_M2_PIN;
  HAL_GPIO_Init(LCD_ID_M2_GPIO_PORT, &gpio_init_structure);

  id |= HAL_GPIO_ReadPin(LCD_ID_M0_GPIO_PORT, LCD_ID_M0_PIN) << 0;
  id |= HAL_GPIO_ReadPin(LCD_ID_M1_GPIO_PORT, LCD_ID_M1_PIN) << 1;
  id |= HAL_GPIO_ReadPin(LCD_ID_M2_GPIO_PORT, LCD_ID_M2_PIN) << 2;
  
  HAL_GPIO_DeInit(LCD_ID_M0_GPIO_PORT, LCD_ID_M0_PIN);
  HAL_GPIO_DeInit(LCD_ID_M1_GPIO_PORT, LCD_ID_M1_PIN);
  HAL_GPIO_DeInit(LCD_ID_M2_GPIO_PORT, LCD_ID_M2_PIN);
...
int32_t BSP_LCD_Init(uint32_t Instance)
{
  ...
  else
  {
    ...
    /* The EK79001 LCD 1024x600 is selected */
    /* Timing Configuration */
    hLtdcHandler.Init.HorizontalSync = (EK79001_HSYNC - (uint16_t)1);
    hLtdcHandler.Init.VerticalSync = (EK79001_VSYNC - (uint16_t)1);
    hLtdcHandler.Init.AccumulatedHBP = (EK79001_HSYNC + EK79001_HBP - (uint16_t)1);
    hLtdcHandler.Init.AccumulatedVBP = (EK79001_VSYNC + EK79001_VBP - (uint16_t)1);
    hLtdcHandler.Init.AccumulatedActiveH = (EK79001_HEIGHT + EK79001_VSYNC + EK79001_VBP - (uint16_t)1);
    hLtdcHandler.Init.AccumulatedActiveW = (EK79001_WIDTH + EK79001_HSYNC + EK79001_HBP - (uint16_t)1);
    hLtdcHandler.Init.TotalHeigh = (EK79001_HEIGHT + EK79001_VSYNC + EK79001_VBP +
                                    EK79001_VFP - (uint16_t)1);
    hLtdcHandler.Init.TotalWidth = (EK79001_WIDTH + EK79001_HSYNC + EK79001_HBP +
                                    EK79001_HFP - (uint16_t)1);

    /* LCD clock configuration */
    BSP_LCD_ClockConfig(&hLtdcHandler, NULL);

    /* Initialize the LCD pixel width and pixel height */
    hLtdcHandler.LayerCfg->ImageWidth  = EK79001_WIDTH;
    hLtdcHandler.LayerCfg->ImageHeight = EK79001_HEIGHT;
...
int32_t BSP_LCD_DisplayOn(uint32_t Instance)
{
  ...
  else
  {
    ...
    // GPIO_Init_Structure.Pin       = LCD_DISP_CTRL_PIN; /* DISP_CTRL */
...
int32_t BSP_LCD_DrawBitmap(uint32_t Instance, uint32_t Xpos, uint32_t Ypos, uint8_t *pBmp)
{
  ...
  else
  {
    ...
    uint32_t input_color_mode = 0xFFFFFFFFU;
...
__weak void BSP_LCD_MspDeInit(LTDC_HandleTypeDef *hltdc, void *Params)
{
  ...
  /*** LTDC Pins configuration ***/
  /* ('I', 14, AF14) LCD_CLK */
  HAL_GPIO_DeInit(GPIOI, GPIO_PIN_14);

  /* ('I', 12, AF14) LCD_HSYNC */
  HAL_GPIO_DeInit(GPIOI, GPIO_PIN_12);

  /* ('I', 13, AF14) LCD_VSYNC */
  HAL_GPIO_DeInit(GPIOI, GPIO_PIN_13);
  ...
  /* ('J',  1, AF14) LCD_R2 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_1);

  /* ('J',  2, AF14) LCD_R3 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_2);

  /* ('K',  3, AF14) LCD_R4 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_3);
  ...
  /* ('J',  6, AF14) LCD_R7 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_6);

  /* ('J',  7, AF14) LCD_G0 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_7);
  ...
  /* ('J',  12, AF14) LCD_B0 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_12);

  /* ('J',  13, AF14) LCD_B1 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_13);

  /* ('J',  14, AF14) LCD_B2 */
  HAL_GPIO_DeInit(GPIOJ, GPIO_PIN_14);
  ...
  /* ('K',  4, AF14) LCD_B5 */
  HAL_GPIO_DeInit(GPIOK, GPIO_PIN_4);

  /* ('K',  5, AF14)  LCD_B6 */
  HAL_GPIO_DeInit(GPIOK, GPIO_PIN_5);

  /* ('K',  6, AF14) LCD_B7 */
  HAL_GPIO_DeInit(GPIOK, GPIO_PIN_6);
