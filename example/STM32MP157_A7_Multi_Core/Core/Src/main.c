...
//  video_stream_init_buffers((uint8_t*)UVC_FRAMEBUFFER0, (uint8_t*)UVC_FRAMEBUFFER1);
  ...
  while (1)
  {
    ...
    //         return 1;
    //     }
    //     size_t bytes_written = fwrite((uint8_t *)uvc_ready_framebuffer_ptr, 1, uvc_ready_frame_length, fp);
    //     if (bytes_written != uvc_ready_frame_length) {
